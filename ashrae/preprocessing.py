...
class Processor:
    ...
    def __call__(self, df_core:pd.DataFrame, df_building:pd.DataFrame=None,
        ...
        print(f'Reduced samples by {self.n - len(df_core)} rows = {(self.n - len(df_core))/self.n*100:.2f} %')
...
def remove_0s_meter0(self:Processor):
    ...
    if self.is_train and self.t_train is not None:
        ok = ok | ~self.t_in_train_set
    print(f'Removing 0s for meter 0: removing {(~ok).sum()} data points = {(~ok).sum()/len(ok)*100:.2f} %')
    self.df_core = self.df_core.loc[ok,:]
    return self.df_core
...
def remove_not_summer_0s_meter_2_and_3(self:Processor):
    ...
    if self.is_train and self.t_train is not None:
        ok = ok | ~self.t_in_train_set
    print(f'Removing 0s for meter 2 and 3 during summer: removing {(~ok).sum()} data points = {(~ok).sum()/len(ok)*100:.2f} %')
    self.df_core = self.df_core.loc[ok,:]
    if add_month:
...
def remove_bad_meter0_readings_of_first_141days(self:Processor):
    ...
    if self.is_train and self.t_train is not None:
        ok = ok | ~self.t_in_train_set
    print(f'Bad readings for meter 0 for the first 141 days for site 0: removing {(~ok).sum()} data points = {(~ok).sum()/len(ok)*100:.2f} %')
    self.df_core = self.df_core.loc[ok,:]
    if add_month:
...
def fix_bid_1099(self:Processor, threshold:float=10.):
    ...
    if self.is_train and self.t_train is not None:
        ok = ok | ~self.t_in_train_set
    print(f'Fixing building_id 1099: removing {(~ok).sum()} data points = {(~ok).sum()/len(ok)*100:.2f} %')
    self.df_core = self.df_core.loc[ok,:]
    return self.df_core
