...
__all__ = ['DATA_PATH', 'get_csvs', 'CSV_NAMES_MAP', 'get_meter_data', 'get_nan_stats', 'show_nans',
           'test_meter_train_and_test_set', 'get_building_data', 'test_building', 'get_weather_data', 'test_weather',
           'load_all']
...
def test_meter_train_and_test_set(df_train:pd.DataFrame, df_test:pd.DataFrame):
    assert len(df_train) == 20216100
    assert len(df_test) == 41697600
...
def test_building(df_building:pd.DataFrame, df_core:pd.DataFrame):
    ...
    assert set(df_core['building_id'].unique()) == set(df_building['building_id'].unique())
...
def load_all(data_path:Path=None) -> dict:
    ...
    ashrae_data['meter_train'] = get_meter_data(csvs['train'])
    ashrae_data['meter_test'] = get_meter_data(csvs['test'])
